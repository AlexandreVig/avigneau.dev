# À propos de moi

Salut, je suis **Alexandre Vigneau**. Je suis étudiant en informatique,
actuellement loin de chez moi.

Je suis en 4ème année à **Epitech Lyon**, et je passe l'année universitaire
2025–2026 à l'étranger à **San Francisco State University**. SFSU m'a permis
d'approfondir les sujets qui me passionnent : le réseau, la sécurité
réseau et l'IA / GenAI.

## Ce qui m'intéresse

Mon cœur de métier est le **développement web full-stack**, mais les rabbit holes
dans lesquels je tombe régulièrement sont le **réseau**, la **sécurité** et
l'**IA/ML**. Je pense qu'un bon modèle de données et un peu de réflexion
peuvent résoudre presque tout. L'architecture et l'optimisation me tiennent
à cœur.

## Expérience

- **Finmoove** : _Développeur full-stack freelance_ · Sep 2025 → présent

  Je continue à développer des fonctionnalités pour [Netmo](https://www.netmo.com/),
  une plateforme SaaS qui aide les entreprises françaises à optimiser leurs
  achats de mobilité en minimisant le coût total de possession dans le cadre
  fiscal français.

- **Finmoove** : _Stage développeur full-stack_ · Mar → Août 2025

  Développement de la plateforme Netmo sur **WeWeb** avec des composants
  **Vue** personnalisés, connectée à un backend **Xano**, et déploiement
  de microservices **Python** sur **GCP**.

- **Abomicro** : _Stage administrateur système_ · Jul → Déc 2023

  Support client, installations logicielles et matérielles, et mise en place
  réseau pour les clients de l'entreprise.

## En ce moment

Je termine mon année à SFSU et je cherche mon prochain stage :
**14 Sep 2026 → 25 Fév 2027**, en alternance 3 jours par semaine en
entreprise et 2 jours à Epitech. Postes **full-stack** de préférence,
frontend, backend, ou les deux. Contrat **stage**, **CDI** ou **CDD**. Si ça correspond à ce que vous cherchez,
ouvrez **Outlook** sur le bureau et envoyez-moi un message !

## En dehors du clavier

Je fais de l'escalade (bloc, principalement) et j'écoute un mix légèrement
chaotique de metal, techno, rap et hyperpop, parfois dans la même playlist.

## Langues

- **Français** : natif
- **Anglais** : courant (865/990 à l'examen interne Epitech)
