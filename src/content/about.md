# About Me

Hi, I'm **Alexandre Vigneau**. I'm a French computer science student, currently a
long way from home.

I'm in my 4th year at **Epitech Lyon**, spending the 2025–2026 academic year
abroad at **San Francisco State University**. SFSU has been a chance to
double down on the topics I keep coming back to networking, network
security, and AI / GenAI.

## What I'm into

My core is **full-stack web development**, but the rabbit holes I keep
falling into are **networking**, **security**, and **AI/ML**. I think a
clean data model and a bit of thinking can solve almost anything. I care
a lot about architecture and optimisation.

## Experience

- **Finmoove** : _Freelance full-stack developer_ · Sep 2025 → present

  Still shipping features to [Netmo](https://www.netmo.com/), a SaaS
  platform that helps French companies optimise their mobility procurement
  by minimising total cost of ownership inside the French tax framework.

- **Finmoove** : _Full-stack web development intern_ · Mar → Aug 2025

  Built the Netmo platform on **WeWeb** with custom **Vue** components,
  wired it to a **Xano** backend, and shipped a handful of **Python**
  microservices on **GCP**.

- **Abomicro** : _Sysadmin intern_ · Jul → Dec 2023

  Client support, software and hardware installs, and network setup for
  the company's customers.

## Right now

I'm wrapping up the year at SFSU and looking for my next internship:
**14 Sep 2026 → 25 Feb 2027**, alternating 3 days a week at a company and
2 days at Epitech. **Full-stack** roles preferred, frontend, backend, or
both. **Internship**, **CDI** or **CDD**. If that lines up with something you're hiring for, open **Outlook** on the desktop and send me a message!

## Off-keyboard

I climb (rock, mostly) and listen to a slightly chaotic mix of metal,
techno, rap, and hyperpop, sometimes in the same playlist.

## Languages

- **French** : native
- **English** : fluent (865/990 on the Epitech internal exam)
